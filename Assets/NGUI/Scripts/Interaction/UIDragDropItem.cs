...
#region Common functionality
	...
	{
		Debug.Log("[UIDragDropItem] OnDragStart : gameobject > " + gameObject.name);
	...
	protected virtual void StartDragging ()
	{
		if (!mDragging)
		{
			if (cloneOnDrag)
			{
				...
				UIDragDropItem item = (UIDragDropItem) clone.GetComponent(this.GetType());// <UIDragDropItem>(); //farhan
				item.mDragging = true;
				item.Start();
	...
	{
		Debug.Log("[UIDragDropItem] OnDragDropStart : gameobject > " + gameObject.name);
...
//		Debug.LogError("repoistioning - - - - - - - ");
	...
	{
		Debug.Log("[UIDragDropItem] OnDragDropRelease : gameobject > " + gameObject.name+" "+cloneOnDrag);
		...
		if (!cloneOnDrag)
		{
			...
			Debug.LogError((surface == null)	+" - surface is null");
			if (container != null)
			{
				Debug.Log("Valid UIDragDropContainer ! > obj name > " + surface.name);
			...
			{
				Debug.Log("NO valid UIDragDropContainer found > obj name > " + surface.name);
		...
		else OnCloneDragRelease(surface);
	}

	protected virtual void OnCloneDragRelease(GameObject surface)
	{
