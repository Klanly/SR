...
//		Debug.Log("New Thread Created with name: + " + thread.Name + " and id: " + thread.ManagedThreadId);
		thread.Start();
	}
	...
	public void CancelAll() {
		lock (requestQueue) {
			...
			RequestType[] reqArray = requestQueue.ToArray();
			int length = reqArray.Length;
...
//					Log.Error("Unhandled Exception ProcessRequest, no callback is going to be called!. Error: "+ e.ToString()); 
					//Debug.Log("Unhandled Exception ProcessRequest, no callback is going to be called!. Error: "+ e.ToString());
					failed = true;
				}
...
//		Debug.Log("This only runs on the Editor: Killing thread: " + GetType().Name + "Thr"); 
		thread.Abort();
	}
#endif
}
