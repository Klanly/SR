...
public class IOSNativePostProcess  {
	...
	public static void OnPostprocessBuild(BuildTarget target, string pathToBuiltProject) {
		...
		if(IOSNativeSettings.Instance.EnableGameCenterAPI) {
			...
			if(!SA.IOSDeploy.ISD_Settings.Instance.ContainsPlistVarWithName(UIRequiredDeviceCapabilities.Name)) {
		...
		if(IOSNativeSettings.Instance.EnableMediaPlayerAPI) {
			...
			if(!SA.IOSDeploy.ISD_Settings.Instance.ContainsPlistVarWithName(NSAppleMusicUsageDescription.Name)) {
		...
		if(IOSNativeSettings.Instance.EnableCameraAPI) {
			...
			if(!SA.IOSDeploy.ISD_Settings.Instance.ContainsPlistVarWithName(NSCameraUsageDescription.Name)) {
			...
			if(!SA.IOSDeploy.ISD_Settings.Instance.ContainsPlistVarWithName(NSPhotoLibraryUsageDescription.Name)) {
		...
		if(IOSNativeSettings.Instance.EnableContactsAPI) {
			...
			if(!SA.IOSDeploy.ISD_Settings.Instance.ContainsPlistVarWithName(NSContactsUsageDescription.Name)) {
