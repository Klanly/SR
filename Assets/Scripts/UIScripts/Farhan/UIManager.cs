...
public class UIManager : Singleton<UIManager> {
	...
	public Mask _mask;
	public const string battleUIPrefabPath = "UIPanels/NGInGameBattle";
	
	//public RaidsUI _raidUIPrefab;
	public const string _raidUIPrefabPath = "UIPrefabs/NGUI/RaidsUI";

	public const string _guildChatUIPrefabPath = "UIPrefabs/NGUI/GuildChatPanel";
	#endregion
	...
	public GameObject guildsSwfGameobject;
...
//		}
	...
	public void OnUsernameListener(string username) 
	{
		...
		else if(guildUI == null)
		{
			...
			NGUITools.SetActive(guildUI.gameObject, true);
		}
		Debug.LogError((guildUI == null) ? "is null so creating guild ui" : "is not null so it be setting active" + " and username = "+username);
	}
	...
	private NGGameUI gameuiPanel;
	public void LoadGeneralSwf(GameManager.GeneralSwfLoaded del)
	{
		if(generalSwf == null)
		{
			gameuiPanel = NGUITools.AddChild(this.gameObject, gameUIPrefab).GetComponent<NGGameUI>();
	...
	private NGBattleUIScript battleUIPanel;
	public void CreateGameHud()
	{
		if(hud == null)
		{
			battleUIPanel = NGUITools.AddChild(this.gameObject, Resources.Load(battleUIPrefabPath) as GameObject).GetComponent<NGBattleUIScript>();
	...
	private NGLevelUIScript leveluiPanel;
	public void CreateLevelScene()
	{
		if(levelScene == null)
		{
			leveluiPanel = NGUITools.AddChild(this.gameObject, levelUIPrefab).GetComponent<NGLevelUIScript>();
...
//		TutorialManager.instance.GetComponent<dfPanel>().BringToFront();
	...
	private NGRaidsUI raidUI;
	
	public NGRaidsUI CreateRaidUI(UI_Raid.LeaderBoardListener leaderboardListener)
	{
		UnityEngine.Object asset = Resources.Load(_raidUIPrefabPath);
		raidUI = NGUITools.AddChild(this.gameObject, asset as GameObject).GetComponent<NGRaidsUI>();
	...
	public void LoadGuildUI()
	{
		...
		var del = new User.UsernameChangeDelegate(this.OnUsernameListener);
		GameManager._gameState.User.usernameChangeEvent -= del;
		GameManager._gameState.User.usernameChangeEvent += del;
		OnUsernameListener(GameManager._gameState.User.username);
	}
	...
	public void CreateGuildUI(System.Action ack)
	{
		if(guildUI == null)
		{
			UnityEngine.Object asset = Resources.Load(_guildChatUIPrefabPath);
			guildUI = NGUITools.AddChild(this.gameObject, asset as GameObject).GetComponent<UI_Guild>();
			//guildUI.OnNewMessageReceived = ;
...
//		Debug.Log("CameraAspect ==> " + cameraAspect);
	...
	public bool isResolution3by2
	{
		...
		{
			float cameraAspect = (float)Screen.width/(float)Screen.height;
			if(cameraAspect >= 1.45f && cameraAspect < 1.7f)
//			if(cameraAspect > 1.45f && cameraAspect < 1.55f)
	...
	{
		UnityEngine.Object asset = Resources.Load("UIPrefabs/NGUI/MainScreenMenu");
		NGMainScreenMenu menu = NGUITools.AddChild(this.gameObject, asset as GameObject).GetComponent<NGMainScreenMenu>();
		menu.Show(singleplayerButtonCB, multiplayerButtonCB);
//
