...
public class CarouselTransmuteCoverFlow : MonoBehaviour 
{
	...
	public dfLabel NextLevel;
	
//	public dfLabel RuneDescription;  //added for runes only. name is ring just to be consisitent
...
//		{
//			
//			TransmuteName.Text = gameObject.transform.GetChild(selectedIndex).GetComponent<item>().upgradeDescription;
//			TransmuteLevel.Text = "Level " +gameObject.transform.GetChild(selectedIndex).GetComponent<item>().upgradeLevel.ToString();
//			NextLevel.Text = "Level " +gameObject.transform.GetChild(selectedIndex).GetComponent<item>().NextupgradeLevel.ToString();
//			gameObject.transform.GetChild(selectedIndex).GetComponent<item>().UpgradeStats(gameObject.transform.GetChild(selectedIndex).GetComponent<dfPanel>());
//			gameObject.transform.parent.parent.GetComponent<Upgradeui>().currentObject = gameObject.transform.GetChild(selectedIndex).GetComponent<item>().upgradeID;
//		}
		
		if(!isUpgrade)
		{
			TransmuteName.Text = gameObject.transform.GetChild((gameObject.transform.childCount-1) - selectedIndex).GetComponent<DragTransmute>()._model.ItemName();
			TransmuteLevel.Text = "Level " +gameObject.transform.GetChild((gameObject.transform.childCount-1) - selectedIndex).GetComponent<DragTransmute>()._model.skullLevel;
	...
	public void gotoTransmute(int index)
	{
	...
	private void layoutSlidesForEditor()
	{
		...
		for( int i = 0; i < controls.Count; i++ )
		{
			...
			controls[ i ].transform.localRotation = new Quaternion(0,0,-90,0);		
	...
	if( currentX == null || selectedIndex != lastSelected )
		...
		for( int i = 0; i < count; i++ )
		{
			...
			if( Application.isPlaying )
			{
				...
				var rot = Quaternion.Euler( 0, calcHorzRotation( itemPosition.x ), 90 );
	...
	public GameObject ObjectAtSelectedIndex()
	{		
		return gameObject.transform.GetChild((gameObject.transform.childCount-1)- selectedIndex).gameObject;
	}
