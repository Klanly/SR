...
public class CarouselCoverFlow : MonoBehaviour 
{
	...
	public void Update()
	{
		...
		if(!isUpgrade)
		{
			RingName.Text = gameObject.transform.GetChild((gameObject.transform.childCount-1) - selectedIndex).GetComponent<DragRing>()._ringModel.ItemName();
			RingLevel.Text = "Level " +gameObject.transform.GetChild((gameObject.transform.childCount-1) - selectedIndex).GetComponent<DragRing>()._ringModel.skullLevel;
		}
	}
